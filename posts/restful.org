...
- [[#特殊信息][特殊信息]]
  - [[#大数值字段---字符串][大数值字段 - 字符串]]
  - [[#日期和时间---iso8601][日期和时间 - ISO8601]]
- [[#基本实体-和-扩展实体][~基本实体~ 和 ~扩展实体~]]
  - [[#基本实体][基本实体]]
  - [[#扩展实体][扩展实体]]
- [[#uri-和-url][URI 和 URL]]
- [[#命名风格][命名风格]]
- [[#meta][Meta]]
- [[#path][Path]]
  - [[#不使用动词而是使用名词][不使用动词，而是使用名词]]
  - [[#使用单数而不是复数][使用单数，而不是复数]]
  - [[#不使用修饰词][不使用修饰词]]
  - [[#一致的模式][一致的模式]]
  - [[#current--all][~$current~ , ~$all~]]
- [[#header][Header]]
  - [[#关于-x--开头的header][关于 ~X-~ 开头的Header]]
...
- [[#query][Query]]
  ...
  - [[#分页---page-和-count-推荐][分页 - ~$page~ 和 ~$count~ （推荐）]]
  - [[#分页---offset-和-limit][分页 - ~$offset~ 和 ~$limit~]]
  - [[#分页---cursor-和-count][分页 - ~$cursor~ 和 ~$count~]]
  - [[#属性挑选---pick][属性挑选 - ~$pick~]]
  - [[#属性排除---omit][属性排除 - ~$omit~]]
  - [[#属性扩展---内嵌---embed][属性扩展 - 内嵌 - ~$embed~]]
  - [[#版本---ver][版本 - ~$ver~]]
- [[#body][Body]]
  - [[#关于服务端生成的内容的属性项的特殊说明][关于服务端生成的内容的属性项的特殊说明]]
  - [[#关于-json-中-数据类型-和-默认值-的说明][关于 ~JSON~ 中 ~数据类型~ 和 ~默认值~ 的说明]]
  - [[#单实体][单实体]]
  - [[#多实体][多实体]]
- [[#特殊操作示例][特殊操作示例]]
  - [[#针对vip用户开启自动置顶的功能][针对vip用户开启自动置顶的功能]]
  - [[#所有vip用户写的文章的所有评论中找出点赞数是4星以上的评论][所有vip用户写的文章的所有评论中找出点赞数是4星以上的评论]]
  - [[#获取当前登陆用户的基本信息][获取当前登陆用户的基本信息]]
  - [[#获取当前登陆用户的角色][获取当前登陆用户的角色]]
  - [[#退出登陆][退出登陆]]
  - [[#文章的收藏和取消收藏][文章的收藏和取消收藏]]
  - [[#查询条件非常复杂的搜索功能][查询条件非常复杂的搜索功能]]
- [[#登陆身份验证退出登陆][登陆、身份验证、退出登陆]]
  - [[#jwt][JWT]]
  - [[#cookie][Cookie]]
...
- [[#分片传输][分片传输]]
- [[#特殊网络环境][特殊网络环境]]
  - [[#x-http-method-override-1][X-HTTP-Method-Override]]
