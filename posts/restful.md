...
大小写的差别消失之后，靠计算机来辨别原本存在的那些词汇边界会变得比较困难，即便是人有的时候也需要花费精力才能辨别。

虽然目前不区分大小写的系统已经不是很多，但市面上仍然有不可忽视的存量。

眼下业界被认同的惯例是**“尽量以符合客户端所使用的语言的命名规范来设计API协议”**（考虑到客户端的开发者体验、性能成本等因素），但我们也要考虑服务端会由各式各样的多种客户端进行消费，而这些客户端极有可能不是用同一种语言实现，也非常大概率命名风格也不一样。
...
`Meta`信息的标识经常与《实体》和《实体属性》标识一起出现，所以我们需要使用特殊字符防止它们和`Meta`信息的冲突。

计算机语言中使用比较多的有《`_`》（下划线）和《`$`》（美元符号）。《`$`》会和《`_`》一样好识别，但优势是上下比较对称，更好看一些。
...
所以，相比**《`/:实体集合/:实体编号`》**，我更推荐**《`/:实体类型/:实体编号`》**这样的模式。
...
现代主流浏览器端的解决方案中几乎全面放弃默认以该格式提交数据，而是建议让开发者使用`application/json`。
...
其实希望通过参数这般复杂的查询条件时，你的查询条件很有可能变得进一步复杂，以上列举的仅仅是比较运算符，除了这些，或许你还需要定义逻辑运算符或如“in”等关系运算符。
...
永远不要在发生服务端异常的时候，假装没有发生服务端异常，用`2XX`, `4XX`系列状态码响应客户端。
